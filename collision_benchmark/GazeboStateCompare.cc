...
bool GazeboStateCompare::Equal(const WorldState& s1, const WorldState& s2, const Tolerances& tolerances, const bool checkLights)
{
  ...
  if (s1.Insertions().size() > 0)  // already checked that insertions sizes of both states are equal
         ...
         ++iter_str1, ++iter_str2)
    {
      ...
      {
#ifdef DEBUG
        std::cout<<"Insertion not equal: "<<std::endl<<*iter_str1<<std::endl<<*iter_str2<<std::endl;
#endif
        return false;
      }
    }
  if (s1.Deletions().size() > 0)  // already checked that deletions sizes of both states are equal
         ...
         ++iter_str1, ++iter_str2)
    {
      ...
      {
#ifdef DEBUG
        std::cout<<"Deletion not equal: "<<std::endl<<*iter_str1<<std::endl<<*iter_str2<<std::endl;
#endif
        return false;
      }
    }
  ...
  if (s1.GetModelStates().size() > 0)  // already checked that modelStates sizes of both states are equal
         ...
         ++iter_mdl1, ++iter_mdl2)
    {
      ...
      {
#ifdef DEBUG
        std::cout<<"Model state names not equal: "<<iter_mdl1->first<<", "<<iter_mdl2->first<<std::endl;
#endif
        return false;
      }
    }
  ...
  if (checkLights && s1.LightStates().size() > 0)  // already checked that lightStates sizes of both states are equal
         ...
         ++iter_lt1, ++iter_lt2)
    {
      ...
      {
#ifdef DEBUG
        std::cout<<"Light names not equal: "<<iter_lt1->first<<", "<<iter_lt2->first<<std::endl;
#endif
        return false;
      }
    }
...
bool GazeboStateCompare::Equal(const gazebo::physics::ModelState& s1, const gazebo::physics::ModelState& s2, const Tolerances& tolerances)
{
  ...
  {
#ifdef DEBUG
    std::cout<<"Model names not equal: "<<s1.GetName()<<", "<<s2.GetName()<<std::endl;
#endif
    return false;
  }
  ...
  if (s1.GetJointStates().size() > 0)  // already checked that lightStates sizes of both states are equal
         ...
         ++iter_jnt1, ++iter_jnt2)
    {
      ...
      {
#ifdef DEBUG
        std::cout<<"Joint names not equal: "<<iter_jnt1->first<<", "<<iter_jnt2->first<<std::endl;
#endif
        return false;
      }
    }
  ...
  if (s1.NestedModelStates().size() > 0)  // already checked that modelStates sizes of both states are equal
         ...
         ++iter_mdl1, ++iter_mdl2)
    {
      ...
      {
#ifdef DEBUG
        std::cout<<"Nested model names not equal: "<<iter_mdl1->first<<", "<<iter_mdl2->first<<std::endl;
#endif
        return false;
      }
    }
  ...
  if (s1.GetLinkStates().size() > 0)
         ...
         ++iter_lnk1, ++iter_lnk2)
    {
      ...
      {
#ifdef DEBUG
        std::cout<<"Link states not equal: "<<iter_lnk1->first<<", "<<iter_lnk2->first<<std::endl;
#endif
        return false;
      }
    }
  ...
  if (s1.GetJointStates().size() > 0)
         ...
         ++iter_jnt1, ++iter_jnt2)
    {
      ...
      {
#ifdef DEBUG
        std::cout<<"Joint states not equal: "<<iter_jnt1->first<<", "<<iter_jnt2->first<<std::endl;
#endif
        return false;
      }
    }
...
bool GazeboStateCompare::Equal(const gazebo::physics::LinkState& s1, const gazebo::physics::LinkState& s2, const Tolerances& tolerances)
{
  if (s1.GetName() != s2.GetName())
  {
    return false;
  ...
  if (tolerances.CheckLinkCollisionStates && s1.GetCollisionStates().size() != s2.GetCollisionStates().size())
  {
#ifdef DEBUG
       ...
       ++iter1, ++iter2)
  {
    ...
    {
#ifdef DEBUG
      std::cout<<"CollisionState name not equal: "<<s1.GetName()<<", "<<s2.GetName()<<std::endl;
#endif
      return false;
    }
...
bool GazeboStateCompare::Equal(const gazebo::physics::JointState& s1, const gazebo::physics::JointState& s2, const Tolerances& tolerances)
{
  ...
  {
#ifdef DEBUG
    std::cout<<"JointState name not equal: "<<s1.GetName()<<", "<<s2.GetName()<<std::endl;
#endif
    return false;
  }

...
class Shape
{
  ...
  // The directory into which data is written when GetShapeSDF() is
  // called and elements are included which rely on a URI, e.g. meshes.
  ...
  // Subdirectory to \e RESOURCE_OUT_DIR into which the meshes will be written.
  // This path will be included in the URI.
  ...
  /// The static variables MESH_OUT_DIR and MESH_SUB_DIR can be used to change
  /// the directory it to which it writes the files.
  ///
  /// \sa SupportLowRes()
  ...
  /// \param uriOnlyWithSubdir if there are SDF elements which rely on data
  ///    being written to file, it is possible to enforce writing to a
  ///    path relative to RESOURCE_OUT_DIR by setting this parameter to
  ///    true. The URI will then be
  ///    ``file://<RESOURCE_SUB_DIR>/<generated_filename>``.
  ///    If set to false, the URI will be
  ///    ``file://<RESOURCE_OUT_DIR>/<RESOURCE_SUB_DIR>/<generated_filename>``.
  public: virtual sdf::ElementPtr
                  GetShapeSDF(bool detailed=true,
                              bool uriOnlyWithSubdir=false) const = 0;

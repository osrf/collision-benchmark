...
#if 0
////////////////////////////////////////////////////////////////
void StaticTestFramework::AABBTestEquivalentShapes(
                                   const std::string& mName1Rep1,
                                   const std::string& mName1Rep2,
                                   const std::string& mName2Rep1,
                                   const std::string& mName2Rep2,
                                   const float cellSizeFactor,
                                   const bool interactive,
                                   const std::string& outputPath)
{
  ...
  collision_benchmark::GzAABB aabb1, aabb2;
  ASSERT_TRUE(GetAABBs(mName1, mName2, aabb1, aabb2));
  ...
  BasicState bstate2;
  bstate2.SetPosition(Vector3(grid.min.X(), grid.min.Y(), grid.min.Z()));
  int cnt = worldManager->SetBasicModelState(mName2, bstate2);
  ASSERT_EQ(cnt, numWorlds) << "All worlds should have been updated";
  ...
  for (double z = grid.min.Z(); z < grid.max.Z()+eps; z += cellSizeZ)
  {
    ...
    bstate2.position.y = y;
    bstate2.position.z = z;
    cnt = worldManager->SetBasicModelState(mName2, bstate2);
    ASSERT_EQ(cnt, numWorlds) << "All worlds should have been updated";
    ...
    std::vector<std::string> colliding, notColliding;
    double maxContactDepth;
    ASSERT_TRUE(collision_benchmark::CollisionState(mName1, mName2,
                                                    worldManager, colliding,
                                                    notColliding,
                                                    maxContactDepth));
# if 0
    ...
    if (!colliding.empty()&& ((itCnt % stopX) == 0))
    {
           ...
           it != colliding.end(); ++it)
      {
        if (it != colliding.begin()) str << std::endl;
        std::vector<GzContactInfoPtr> contacts =
          collision_benchmark::GetContactInfo(mName1, mName2,
                                              *it, worldManager);
        str << *it << ": " << VectorPtrToString(contacts);
      }
      RefreshClient(5);
